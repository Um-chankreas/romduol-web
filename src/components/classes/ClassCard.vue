...
const props = defineProps({
  ...
  // Preview list of enrolled students: [{ id, name, avatar_url }]
  students: {
    type: Array,
    default: () => []
  },
  // e.g. "Mon, Wed · 3:00 PM - 4:30 PM" — omitted when no recurring schedule is set.
  scheduleLabel: {
    type: String,
    default: ''
  }
...
const remainingStudents = computed(() =>
  Math.max(0, props.studentCount - previewStudents.value.length)
);
...
const toggleMenu = () => {
  ...
  emit('more-options', props.courseId);
  if (showMenu.value) {
...
const handleDelete = () => {
  showMenu.value = false;
  document.removeEventListener('click', handleClickOutside);
  emit('delete-course', props.courseId, props.title);
};
...
<template>
  <div class=" relative bg-[#ffffff] dark:bg-[#111827] rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100 dark:border-slate-800/80 flex flex-col justify-between min-h-[220px] h-full overflow-hidden text-left select-none">
    ...
    <div class="class-card-menu absolute top-4 right-4 z-20" @click.stop>
      ...
      <div
        v-if="showMenu"
        class="absolute right-0 top-full mt-1 w-40 rounded-xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-lg overflow-hidden py-1"
      >
        <button
          @click="showMenu = false; emit('view-details', courseId)"
          class="w-full text-left px-4 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer"
        >
          View Details
        </button>
        <button
        ...
        <button
          @click="handleDelete"
          class="w-full text-left px-4 py-2.5 text-xs font-semibold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition cursor-pointer"
        >
          Delete Class
        </button>
      </div>
    ...
    <div class="space-y-3 z-10">
      ...
      <!-- Recurring schedule -->
      <div v-if="scheduleLabel" class="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-medium text-xs">
    ...
    <div class="flex items-center justify-between gap-2 pt-4 z-10">
      ...
      <div class="flex items-center -space-x-2 shrink-0">
        <template v-for="(student, i) in previewStudents" :key="student.id || i">
          <img
          ...
          <div
            v-else
            class="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-[#016a36] text-white ring-2 ring-white dark:ring-[#111827] text-[10px] sm:text-[11px] font-bold select-none"
            :title="student.name"
          >
